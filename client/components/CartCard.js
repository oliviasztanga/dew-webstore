...
class CartCard extends Component {
  ...
  handleChange(event) {
    if (event.target.value > this.props.lineitem.option.stock)
      event.target.value = this.props.lineitem.option.stock
    this.setState({
            ...
            <form className="form-inline" onSubmit={this.handleSubmit}>
              <div className="form-group">
                <div className="d-flex flex-row justify-content-start align-items-center flex-wrap">
                  ...
                  />
                  <button
                    className="btn btn-light mr-2 mb-2 flex-shrink-0"
                    type="submit"
                  >
                    Save Quantity
                  </button>
                  <button
                    className="btn btn-light mb-2 flex-shrink-0"
                    type="button"
...
const mapDispatchToProps = dispatch => ({
  editLineItem: (orderId, optionId, quantity) =>
    dispatch(editLineItem(orderId, optionId, quantity)),
